...
class TrainingRestController extends Controller
{
    public function init()
    {
        ...
        register_rest_route( 'api/hr','/training', array(
            'methods' => 'POST',
    ...
    public function post($request){
        ...
        if ($o['id'] > 0) {
            ...
            $updated = $wpdb->update('hr_training', $o, array('id' => $o['id']));
        } else {
            unset($o['id']);
            ...
            $updated = $wpdb->insert('hr_training', $o);
            $o['id'] = $wpdb->insert_id;
            $inserted = 1;
        }
    ...
    public function delete($data){
        ...
        $result = array_map(function ($id) use ($wpdb) {
            return $wpdb->update('hr_training', array('canceled' => 1, 'delete_date' => current_time('mysql')), array('id' => $id));
        }, explode(",", $data['id']));
