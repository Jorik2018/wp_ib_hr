    ...
    protected function task($item)
    {
        ...
        if (empty($product)) {
            ...
            $categories = array_map(function ($e) {
                return $e->term_id;
            }, array_values(array_filter($categories_terms, function ($e) use ($item) {
                return $e->slug == $item['category'];
            })));
            $product->set_category_ids($categories);
            $brands = array_map(function ($e) {
                return $e->term_id;
            }, array_values(array_filter($brands_terms, function ($e) use ($item) {
                return $e->slug == $item['brand'];
            })));
...
class Main extends Bridge
{
    ...
    public function init()
    {
        ...
        $this->add_action('rest_api_init', 'GroupRestController@rest_api_init');
        ...
        $this->add_action('rest_api_init', 'AttentionRestController@init');
        $this->add_action('plugins_loaded', 'AdminController@activate');

        $this->add_action('rest_api_init', 'PersonalRestController@rest_api_init');
        $this->add_action('rest_api_init', 'ResourceRestController@rest_api_init');
        $this->add_action('init','ResourceRestController@init');
        $this->add_action('rest_api_init', 'ServiceRestController@rest_api_init');
        $this->add_action('rest_api_init', 'PayrollRestController@rest_api_init');
        $this->add_action('rest_api_init', 'StatsRestController@rest_api_init');
        $this->add_action('rest_api_init', 'MovimientoRestController@rest_api_init');
        
        $this->add_action('init','PersonalRestController@init');
    }
